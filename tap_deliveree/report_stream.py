...
class ReportStream:
    ...
    def generate_catalog_entry(self):
        ...
        return CatalogEntry(tap_stream_id=stream,
                            stream=stream,
    ...
    def write_schema(self):
        schema = self.generate_schema()
        key_properties = self.report["key_properties"]
        return singer.write_schema(stream_name=self.report["name"],
                                   schema=schema,
                                   key_properties=key_properties)

    @backoff.on_exception(backoff.expo, Exception, max_tries=5, factor=2)
    def get_data(self, config):
        url = f"{config['base_url']}/{self.report['resource']}/{self.report['name']}"
